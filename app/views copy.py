...
def send_otp(mobile,otp):
    ...
    url = "http://api.msg91.com/api/sendotp.php?otp="+otp+"&message="+"Your otp is "+otp +"&mobile="+mobile+"&authkey="+authkey+"&country=91"
...
def otp(request):
    ...
    if request.method == 'POST':
        ...
        profile = OtpModel.objects.filter(mobile=mobile).first()
        if otp == profile.otp:
...
def update_record(request,id,empid):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
                ...
                if profile_pic == None:
                    profile_pic = "image/profile_pic/"+str(saved_id) + '.jpg' 
